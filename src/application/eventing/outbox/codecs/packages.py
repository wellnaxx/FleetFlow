...
class PackageDeliveredEventPayloadCodec(EventPayloadCodec[PackageDelivered]):
    """Encode and decode the version-2 package delivery transition snapshot.

    All seven payload keys are required, including nullable scheduled_arrival.
    IDs are positive integers, statuses use ItemStatus values, and locations
    use LocationCode normalization. Scheduled arrival remains a naive business
    timestamp distinct from occurrence and recording times. Encoding trusts
    typed event fields; decoding validates serialized values without consulting
    live entities or reapplying delivery eligibility rules.
    """

    @property
    def event_class(self) -> type[PackageDelivered]:
        """Return the concrete package-delivered event class."""
        return PackageDelivered

    @property
    def event_type(self) -> str:
        """Return the stable persisted identity ``package_delivered``."""
    ...
    def encode(self, event: PackageDelivered) -> JSONObject:
        """Serialize the delivery's before/after snapshot into a fresh JSON object.
        ...
        Returns:
            Integer package and route IDs, string status values and locations,
            and an ISO-formatted scheduled arrival or JSON null. Microseconds
            are preserved; event and envelope metadata are excluded.
        """
        return {
            "package_id": event.package_id,
            "route_id": event.route_id,
    ...
    def decode(
        ...
        *,
        ...
        """Validate a delivery payload and restore its original event metadata.

"""Structured result returned by truck assignment policy evaluation."""
...
class TruckAssignmentDecision:
    ...
    reason: TruckAssignmentRejectionReason | None = None
    ...
    @classmethod
    def accept(cls) -> TruckAssignmentDecision:
        """Return an accepted assignment decision."""
        return cls()

    @classmethod
    def reject(cls, reason: TruckAssignmentRejectionReason, message: str) -> TruckAssignmentDecision:
        ...
        Raises:
            DomainValidationError: If ``reason`` or ``message`` has an invalid
                runtime type, or ``message`` is empty or blank.
        """
...
def _require_runtime_types(reason: object, message: object) -> None:
    ...
    Args:
        reason: Candidate machine-readable rejection reason.
        message: Candidate human-readable rejection message.

    Raises:
        DomainValidationError: If a non-null reason is not a
            ``TruckAssignmentRejectionReason`` or a non-null message is not a
            string.
    """
    if reason is not None and not isinstance(reason, TruckAssignmentRejectionReason):
        raise DomainValidationError("Assignment rejection reason must be a TruckAssignmentRejectionReason.")

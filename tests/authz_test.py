...
class Authz_Should(unittest.TestCase):
    ...
    def test_has_true_when_user_role_allows_permission(self):
    ...
    @patch('src.core.authz.ROLE_PERMISSIONS', {})
    def test_has_false_when_no_current_user_or_role_not_mapped(self):
    ...
    def test_requires_allows_and_forwards_args_kwargs(self):
        calls = {}

        class Target:
            def __init__(self):
                # permissive authz
                self.authz = SimpleNamespace(has=lambda p: p == Permission.ROUTE_REMOVE)

            @requires(Permission.ROUTE_REMOVE)
            def do_work(self, a, b=0, *, k=None):
                calls['args'] = (a, b, k)
                return "ok"
    ...
    def test_requires_raises_with_missing_permission_or_no_authz(self):
        class NoAuthz:
            @requires(Permission.PACKAGE_CREATE)
            def action(self):  # pragma: no cover - name only used for metadata
                return "x"
        ...
        class WithAuthz:
            def __init__(self):
                self.authz = SimpleNamespace(has=lambda p: False)
    ...
    def test_requires_all_allows_when_all_present(self):
        needed = {Permission.PACKAGE_CREATE, Permission.ROUTE_REMOVE}
        class OK:
            def __init__(self):
                self.authz = SimpleNamespace(has=lambda p: p in needed)
    ...
    def test_requires_all_raises_when_missing_or_no_authz(self):
        class NoAuthz:
            @requires_all(Permission.PACKAGE_CREATE, Permission.ROUTE_REMOVE)
            def go(self):
                return 1
        ...
        class MissingOne:
            def __init__(self):
                ...
                self.authz = SimpleNamespace(has=lambda p: p == Permission.PACKAGE_CREATE)

...
class CreateRoute_Should(unittest.TestCase):
    ...
    @patch('src.commands.create_route.parse_departure_from_tail')
    @patch('src.commands.create_route.validate_params_count')
    def test_success_unscheduled(self, mock_validate, mock_parse):
        # Arrange
        locs = ["SYD", "MEL", "ADL"]
        mock_parse.return_value = (locs, None)
        cmd = self.make_cmd(["SYD", "MEL", "ADL"])
        route = SimpleNamespace(route_id=42, total_distance_km=1365)
        cmd._app_data.create_route.return_value = route
        ...
        cmd._app_data.create_route.assert_called_once_with(locs, None)
        self.assertEqual(
            result,
    ...
    @patch('src.commands.create_route.parse_departure_from_tail')
    @patch('src.commands.create_route.validate_params_count')
    def test_success_with_departure_datetime(self, mock_validate, mock_parse):
        # Arrange
        locs = ["SYD", "MEL"]
        ...
        cmd._app_data.create_route.assert_called_once_with(locs, departure)
        self.assertEqual(
            result,
    ...
    @patch('src.commands.create_route.parse_departure_from_tail')
    @patch('src.commands.create_route.validate_params_count')
    def test_message_joins_locations_correctly(self, mock_validate, mock_parse):
        # Arrange: ensure join uses ' -> '
        locs = ["A", "B", "C", "D"]
        mock_parse.return_value = (locs, None)
        cmd = self.make_cmd(locs)
        cmd._app_data.create_route.return_value = SimpleNamespace(route_id=1, total_distance_km=10)
    ...
    @patch('src.commands.create_route.parse_departure_from_tail')
    def test_param_count_error_bubbles_and_stops(self, mock_parse):
        # Arrange: validate_params_count raises
        cmd = self.make_cmd(["ONLYONE"])
        with patch('src.commands.create_route.validate_params_count', side_effect=ValueError("need at least 2")):
            with self.assertRaises(ValueError) as ctx:
                cmd.execute()
        self.assertIn("need at least 2", str(ctx.exception))
        mock_parse.assert_not_called()
        cmd._app_data.create_route.assert_not_called()

    @patch('src.commands.create_route.parse_departure_from_tail')
    @patch('src.commands.create_route.validate_params_count')
    def test_parse_result_is_passed_verbatim_to_app(self, mock_validate, mock_parse):
        ...
        cmd._app_data.create_route.return_value = SimpleNamespace(route_id=9, total_distance_km=123)

        _ = cmd.execute()

        # Assert
        called_locs, called_dep = cmd._app_data.create_route.call_args[0]
        self.assertEqual(called_locs, loc_tokens)
        self.assertEqual(called_dep, dep)

    @patch('src.commands.create_route.parse_departure_from_tail')
    @patch('src.commands.create_route.validate_params_count')
    def test_downstream_error_propagates(self, mock_validate, mock_parse):
        mock_parse.return_value = (["SYD", "MEL"], None)
        cmd = self.make_cmd(["SYD", "MEL"])
        cmd._app_data.create_route.side_effect = RuntimeError("db failure")

...
class Crypto_Should(unittest.TestCase):
    ...
    @patch("src.core.crypto.os.urandom", return_value=b"SALT"*4)  # 16 bytes
    @patch("src.core.crypto.PBKDF2_ITERATIONS", 100)  # speed up
    def test_hash_password_generates_expected_shape_and_min_len(self, *_):
        ...
        with self.assertRaises(ValueError):
            hash_password(12345678)   # not a str

    @patch("src.core.crypto.os.urandom", return_value=b"A"*16)
    @patch("src.core.crypto.PBKDF2_ITERATIONS", 50)
    def test_hash_password_different_salts_produce_different_hashes(self, *_):
        ph1 = hash_password("SamePassword123")
        # change salt:
        with patch("src.core.crypto.os.urandom", return_value=b"B"*16):
    ...
    @patch("src.core.crypto.os.urandom", return_value=b"\xFF"*16)  # fixed salt
    @patch("src.core.crypto.PBKDF2_ITERATIONS", 100)               # fast & deterministic
    def test_verify_password_true_and_false(self, *_):
    ...
    @patch("src.core.crypto.hmac.compare_digest", return_value=True)
    @patch("src.core.crypto.hashlib.pbkdf2_hmac", return_value=b"\x11"*32)
    def test_verify_password_uses_compare_digest(self, pbkdf2_mock, cd_mock):
        stored = PasswordHash(algo="sha256", iterations=123, salt_b64=self._b64(b"S"*16), hash_b64=self._b64(b"\x11"*32))
        ...
        args, kwargs = pbkdf2_mock.call_args
        self.assertEqual(args[0], "sha256")
        self.assertEqual(args[3], 123)

    def test_passwordhash_serialize_and_parse_roundtrip(self):
        ph = PasswordHash(algo="sha256", iterations=200000, salt_b64=self._b64(b"X"*16), hash_b64=self._b64(b"Y"*32))
        s = ph.serialize()

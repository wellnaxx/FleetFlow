...
class AuthLogout_Should(unittest.TestCase):
    ...
    def test_execute_calls_logout_and_returns_message(self) -> None:
        cmd = self.make_cmd()
        result = cmd.execute()
        cmd._use_case.execute_current_session.assert_called_once_with()  # type: ignore[reportUnknownMemberType]
        self.assertEqual(result, "Logged out.")

    def test_execute_propagates_errors_from_auth(self) -> None:
        cmd = self.make_cmd()
        cmd._use_case.execute_current_session.side_effect = RuntimeError("session not found")  # type: ignore[reportAttributeAccessIssue]

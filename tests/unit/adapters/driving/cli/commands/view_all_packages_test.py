...
class TestViewAllPackages_Should(unittest.TestCase):
    def make_cmd(self, *, authorized: bool = True) -> ViewAllPackages:
        ...
        cmd._view_all_packages_use_case = MagicMock()  # type: ignore[reportAttributeAccessIssue]
    ...
    def test_execute_without_permission_raises(self) -> None:
        ...
        cmd._view_all_packages_use_case.execute.assert_not_called()  # type: ignore[reportUnknownMemberType]

    def test_no_packages_available(self) -> None:
        cmd = self.make_cmd(authorized=True)
        cmd._view_all_packages_use_case.execute.return_value = []  # type: ignore[reportAttributeAccessIssue]

        result = cmd.execute()

        self.assertEqual(result, "No packages.")
        cmd._view_all_packages_use_case.execute.assert_called_once_with()  # type: ignore[reportUnknownMemberType]

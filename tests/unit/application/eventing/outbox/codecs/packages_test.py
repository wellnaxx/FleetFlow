...
class PackageDeliveredCodecShould(unittest.TestCase):
    def setUp(self) -> None:
        self.codec = PackageDeliveredEventPayloadCodec()
        self.payload: JSONObject = {
            "package_id": 7,
            "route_id": 19,
            "previous_status": ItemStatus.IN_PROGRESS.value,
            "new_status": ItemStatus.DONE.value,
            "previous_location": "SYD",
            "new_location": "MEL",
            "scheduled_arrival": ARRIVAL.isoformat(),
        }

    def decode(self, payload: JSONObject) -> PackageDelivered:
    ...
    def test_exact_wire_contract_and_json_round_trip(self) -> None:
        for arrival in (None, ARRIVAL, OCCURRED_AT, OCCURRED_AT - timedelta(days=1)):
            with self.subTest(arrival=arrival):
                ...
                expected["scheduled_arrival"] = arrival.isoformat() if arrival is not None else None
                encoded = self.codec.encode(event)
                self.assertEqual(encoded, expected)
                for field in ("previous_status", "new_status", "previous_location", "new_location"):
                ...
                self.assertIs(type(restored), PackageDelivered)
                self.assertEqual(restored, event)
                self.assertIs(restored.previous_status, ItemStatus.IN_PROGRESS)
                self.assertIs(restored.new_status, ItemStatus.DONE)
    ...
    def test_validates_both_ids_without_coercion(self) -> None:
    ...
    def test_preserves_unchanged_location(self) -> None:
        payload = {**self.payload, "previous_location": "MEL"}
        event = self.decode(payload)
    ...
    def test_rejects_status_names_unknown_values_and_wrong_types(self) -> None:
        ...
        for field in ("previous_status", "new_status"):
            for value in wrong_types:
                with self.subTest(field=field, value=value), self.assertRaisesRegex(TypeError, field):
                    self.decode({**self.payload, field: value})
            for value in ("TODO", "IN_PROGRESS", "DONE", "unknown", "", " Done "):
    ...
    def test_rejects_arrival_non_string_values_including_datetime_objects(self) -> None:
        ...
        for value in values:
            with self.subTest(value=value), self.assertRaisesRegex(TypeError, "scheduled_arrival"):
                self.decode({**self.payload, "scheduled_arrival": cast(JSONValue, value)})
    ...
    def test_rejects_invalid_or_timezone_aware_arrivals(self) -> None:
        for value in (
            ...
            with self.subTest(value=value), self.assertRaisesRegex(ValueError, "scheduled_arrival"):
                self.decode({**self.payload, "scheduled_arrival": value})

    def test_invalid_arrival_text_preserves_parse_error_as_cause(self) -> None:
        with self.assertRaisesRegex(ValueError, "scheduled_arrival") as raised:
            self.decode({**self.payload, "scheduled_arrival": "invalid"})
    ...
    def test_payload_and_event_remain_independent(self) -> None:
        ...
        self.payload["route_id"] = 999
        self.assertEqual(event.route_id, 19)
        encoded = self.codec.encode(event)
        encoded["previous_location"] = "MEL"
        self.assertEqual(self.codec.encode(event), original)

    def test_registry_resolves_version_two_without_conflicting_with_other_package_codecs(self) -> None:
        ...
        registry.register(PackageDelivered, self.codec)
        event = self.decode(self.payload)
        adapter = registry.for_identity("package_delivered", 2)
        self.assertIs(adapter, registry.for_event(event))
        self.assertIs(adapter.event_class, PackageDelivered)
        self.assertEqual(adapter.event_version, PackageDelivered.event_version)
        for name in ("package_created", "package_removed", "package_picked_up"):
        ...
        for version in (1, 3):
            with self.subTest(version=version), self.assertRaises(EventCodecNotFoundError):
                registry.for_identity("package_delivered", version)
...
class PackagePickedUpCodecShould(unittest.TestCase):
    def setUp(self) -> None:
        self.codec = PackagePickedUpEventPayloadCodec()
        self.payload: JSONObject = {
            "package_id": 7,
            "route_id": 19,
            "previous_status": ItemStatus.TODO.value,
    ...
    def decode(self, payload: JSONObject) -> PackagePickedUp:
        return self.codec.decode(
            payload, event_id=EVENT_ID, occurred_at=OCCURRED_AT, recorded_at=RECORDED_AT
        )

    def test_exact_wire_contract_and_json_round_trip(self) -> None:
        for arrival in (None, ARRIVAL, OCCURRED_AT - timedelta(days=1)):
            with self.subTest(arrival=arrival):
                ...
                for field in ("package_id", "route_id"):
                    self.assertIs(type(encoded[field]), int)
                restored = self.decode(cast(JSONObject, json.loads(json.dumps(encoded, allow_nan=False))))
                self.assertIs(type(restored), PackagePickedUp)
                self.assertEqual(restored, event)
                self.assertIs(restored.previous_status, ItemStatus.TODO)
    ...
    def test_preserves_unchanged_location(self) -> None:
        payload = {**self.payload, "previous_location": "SYD"}
        event = self.decode(payload)
        self.assertEqual(event.previous_location, event.new_location)
        self.assertEqual(self.codec.encode(event), payload)
    ...
    def test_registry_resolves_version_two_without_conflicting_with_other_package_codecs(self) -> None:
        ...
        registry.register(PackageCreated, PackageCreatedEventPayloadCodec())
        registry.register(PackageRemoved, PackageRemovedEventPayloadCodec())
        registry.register(PackagePickedUp, self.codec)
        event = self.decode(self.payload)
        adapter = registry.for_identity("package_picked_up", 2)
        self.assertIs(adapter, registry.for_event(event))
        self.assertIs(adapter.event_class, PackagePickedUp)
        self.assertEqual(adapter.event_version, PackagePickedUp.event_version)
        for name in ("package_created", "package_removed"):
            self.assertIsNot(adapter, registry.for_identity(name, 2))
        self.assertEqual(
            adapter.decode(
                adapter.encode(event), event_id=EVENT_ID, occurred_at=OCCURRED_AT, recorded_at=RECORDED_AT
        ...
        for version in (1, 3):
            with self.subTest(version=version), self.assertRaises(EventCodecNotFoundError):
                registry.for_identity("package_picked_up", version)

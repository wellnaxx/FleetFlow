...
class WorldStateAdvancedCodecShould(unittest.TestCase):
    def setUp(self) -> None:
        self.codec = WorldStateAdvancedEventPayloadCodec()
        ...
        for counters in ((2, 7, 4, 3, 5), (0, 0, 0, 0, 0)):
            with self.subTest(counters=counters):
                event = WorldStateAdvanced(
                    ...
                    routes_updated=counters[0],
                    packages_updated=counters[1],
                    trucks_moved=counters[2],
                    trucks_released=counters[3],
                    trucks_reconciled=counters[4],
                )
                expected: JSONObject = dict(zip(self.payload, counters, strict=True))
    ...
    def test_rejects_non_integer_counters_without_coercion(self) -> None:
    ...
    def test_rejects_negative_counters_independently(self) -> None:
        for field in self.payload:
            for value in (-1, -(2**63)):
                with self.subTest(field=field, value=value), self.assertRaisesRegex(ValueError, field):
                    self.decode({**self.payload, field: value})

    def test_accepts_zero_and_large_counters_and_maps_each_to_correct_attribute(self) -> None:
        for field in self.payload:
    ...
    def test_payload_and_event_remain_independent(self) -> None:
        ...
        self.payload["routes_updated"] = 999
        self.assertEqual(event.routes_updated, 2)
        encoded = self.codec.encode(event)
        encoded["trucks_reconciled"] = 0
        self.assertEqual(event.trucks_reconciled, 5)
    ...
    def test_registry_resolves_version_two_and_round_trips_through_adapter(self) -> None:
        ...
        registry.register(WorldStateAdvanced, self.codec)
        event = self.decode(self.payload)
        adapter = registry.for_identity("world_state_advanced", 2)

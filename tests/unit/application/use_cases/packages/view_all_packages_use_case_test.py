...
class ViewAllPackagesUseCase_Should(unittest.TestCase):
    ...
    def test_returns_all_packages(self) -> None:
        ...
        result = self.use_case.execute(ViewAllPackagesQuery())
    ...
    def test_returns_requested_package_page(self) -> None:
        ...
        result = self.use_case.execute(ViewAllPackagesQuery(page=PageQuery(limit=10, offset=20)))
    ...
    def test_rejects_invalid_pagination(self) -> None:
        with self.assertRaises(ValidationError):
            self.use_case.execute(ViewAllPackagesQuery(page=PageQuery(limit=0)))

        with self.assertRaises(ValidationError):
            self.use_case.execute(ViewAllPackagesQuery(page=PageQuery(limit=1, offset=-1)))

        with self.assertRaises(ValidationError):
            self.use_case.execute(ViewAllPackagesQuery(page=PageQuery(offset=1)))
    ...
    def test_returns_requested_package_page_with_total(self) -> None:
        ...
        result = self.use_case.execute(
            ViewAllPackagesQuery(page=PageQuery(limit=10, offset=20, include_total=True))
        )
    ...
    def test_requires_package_view_all_permission(self) -> None:
        ...
        with self.assertRaisesRegex(PermissionError, "Unauthenticated"):
            use_case.execute(ViewAllPackagesQuery())
        ...
        self.assertIs(event.attempted_operation, AuthorizationOperation.PACKAGE_LIST)

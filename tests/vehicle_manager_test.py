...
class VehicleManager_Should(unittest.TestCase):
    ...
    def test_init_builds_fleet_and_disperses_round_robin(self, _get_locs):
        vm = VehicleManager()
    ...
    def test_list_fleet_returns_copy_and_find_by_id(self, _get_locs):
        vm = VehicleManager()
    ...
    def _fake_route(self, *, total_distance, assigned_weight, start_loc,
                    departure_time=None, truck_eta_final=None):
    ...
    def _fake_truck(self, *, capacity, max_range, current_location, assigned_route=None):
    ...
    def test_is_suitable_false_range_too_short(self):
        vm = VehicleManager()
        ...
        ok, reason = vm.is_suitable_for_route(t, r)
        self.assertFalse(ok)
        self.assertIn("range too short", reason)

    def test_is_suitable_false_insufficient_capacity(self):
        vm = VehicleManager()
    ...
    def test_is_suitable_false_wrong_location(self):
        vm = VehicleManager()
    ...
    def test_is_suitable_false_truck_busy_in_window(self):
        vm = VehicleManager()
        dep = datetime(2025, 1, 1, 10, 0)
        # Active route ends at 11:00 >= desired departure => busy
        r, active = self._fake_route(total_distance=100, assigned_weight=0, start_loc="SYD",
                                     departure_time=dep, truck_eta_final=datetime(2025, 1, 1, 11, 0))
        t = self._fake_truck(capacity=1000, max_range=1000, current_location="SYD", assigned_route=active)
        ok, reason = vm.is_suitable_for_route(t, r)
        self.assertFalse(ok)
        self.assertIn("busy", reason)

    def test_is_suitable_false_route_not_scheduled_yet(self):
        vm = VehicleManager()
        # New route has no departure_time; truck already on some route -> reject
        r, active = self._fake_route(total_distance=100, assigned_weight=0, start_loc="SYD",
                                     departure_time=None, truck_eta_final=datetime(2025, 1, 1, 9, 0))
        t = self._fake_truck(capacity=1000, max_range=1000, current_location="SYD", assigned_route=active)
        ok, reason = vm.is_suitable_for_route(t, r)
        self.assertFalse(ok)
        self.assertIn("route not scheduled yet", reason)

    def test_is_suitable_true_when_all_conditions_ok(self):
        vm = VehicleManager()
        dep = datetime(2025, 1, 1, 10, 0)
        # Truck free (no assigned route), enough range/capacity, correct location
        r, _ = self._fake_route(total_distance=100, assigned_weight=900, start_loc="SYD",
                                departure_time=dep, truck_eta_final=None)
    ...
    def test_find_available_for_route_filters_and_sorts(self):
        vm = VehicleManager()
        ...
        allow = {2, 9}
        with patch.object(VehicleManager, "is_suitable_for_route",
                          side_effect=lambda t, route: (t.vehicle_id in allow, "")):
            res = vm.find_available_for_route(route=SimpleNamespace())

...
class ViewRoutesInProgress_Should(unittest.TestCase):
    ...
    def _mk_route(self, text):
        r = MagicMock()
        r.info.return_value = text
        return r

    @patch('src.commands.view_routes_in_progress.datetime')
    def test_no_routes_returns_friendly_message(self, mock_dt):
        fixed_now = datetime(2025, 9, 27, 10, 0, 0)
        mock_dt.now.return_value = fixed_now

        cmd = self.make_cmd()
        cmd._app_data.view_routes_in_progress.return_value = []

        out = cmd.execute()

        cmd._app_data.view_routes_in_progress.assert_called_once_with(now=fixed_now)
        self.assertEqual(out, "No routes in progress.")

    @patch('src.commands.view_routes_in_progress.datetime')
    def test_formats_in_transit_and_at_stop(self, mock_dt):
    ...
    @patch('src.commands.view_routes_in_progress.datetime')
    def test_unknown_pos_kind_includes_only_route_info_and_blank_line(self, mock_dt):
        ...
        cmd._app_data.view_routes_in_progress.return_value = [(route, pos)]
    ...
    @patch('src.commands.view_routes_in_progress.datetime')
    def test_multiple_entries_have_blank_separators(self, mock_dt):
        ...
        cmd._app_data.view_routes_in_progress.return_value = [(r1, p1), (r2, p2)]
        out = cmd.execute()
